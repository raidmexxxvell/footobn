...
exports.pgRepo = {
    ...
    async updateUserRowSafely({ user_id, expectedNotCheckedToday, update }) {
        ...
        try {
            ...
            if (last === expectedNotCheckedToday) {
                ...
                err.code = 'ALREADY_CHECKED_TODAY';
                throw err;
            }
            await client.query(`
        UPDATE users SET
    ...
    async getLeaderboard(limit) {
        const { rows } = await pool.query('SELECT display_name, xp, level FROM users ORDER BY level DESC, xp DESC LIMIT $1', [limit]);
    ...
    async supersedeLowerTierAchievements(userId, newTier) {
        ...
        await pool.query('UPDATE achievements SET superseded=TRUE WHERE user_id=$1 AND (CASE tier WHEN \'bronze\' THEN 1 WHEN \'silver\' THEN 2 WHEN \'gold\' THEN 3 END) < $2 AND superseded=FALSE', [userId, newRank]);
    },
    async upsertAchievement(a) {
        await pool.query(`
    ...
    async getUserAchievements(userId) {
        const { rows } = await pool.query('SELECT user_id, tier, title, description, unlocked_at, superseded FROM achievements WHERE user_id=$1', [userId]);
        return rows.map(r => ({
            user_id: r.user_id,
            tier: r.tier,
            title: r.title,
